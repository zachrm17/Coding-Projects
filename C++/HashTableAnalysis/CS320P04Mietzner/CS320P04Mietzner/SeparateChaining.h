...
public:
	...
	bool contains(const HashedObj & x) const
	{
		...
		probes = 1;
		for (auto it = begin(whichList); (it != end(whichList)) && (*it != x); it++) {
			probes++;
		}
	...
	{
		auto & whichList = theLists[myhash(x)]; // whichlist is a list of hashed objects or buckets
		probes = 1;
		for (auto it = begin(whichList); (it != end(whichList)) && (*it != x); it++) {
			probes++;
		}
	...
	// Both edits go into both functions
